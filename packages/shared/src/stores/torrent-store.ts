...
export const useTorrentStore = create<TorrentStore>((set, get) => ({
  ...
  fetchTorrents: async () => {
    set({ isLoading: true });
    try {
      const res = await getPlatformApi().rpcTorrentGet(TORRENT_LIST_FIELDS as unknown as string[]);
  ...
  fetchRecentlyActive: async () => {
    try {
      const res = await getPlatformApi().rpcTorrentGet(
  ...
  startTorrents: async (ids) => {
    await getPlatformApi().rpcTorrentStart(ids);
    await get().fetchRecentlyActive();
  },

  startNowTorrents: async (ids) => {
    await getPlatformApi().rpcTorrentStartNow(ids);
    await get().fetchRecentlyActive();
  },

  stopTorrents: async (ids) => {
    await getPlatformApi().rpcTorrentStop(ids);
    await get().fetchRecentlyActive();
  },

  verifyTorrents: async (ids) => {
    await getPlatformApi().rpcTorrentVerify(ids);
    await get().fetchRecentlyActive();
  },

  reannounceTorrents: async (ids) => {
    await getPlatformApi().rpcTorrentReannounce(ids);
  },

  removeTorrents: async (ids, deleteData) => {
    await getPlatformApi().rpcTorrentRemove(ids, deleteData);
    set((state) => {
      const newMap = new Map(state.torrents);
  ...
  setTorrentProps: async (ids, props) => {
    await getPlatformApi().rpcTorrentSet(ids, props);
    await get().fetchRecentlyActive();
  },

  moveTorrents: async (ids, location, move) => {
    await getPlatformApi().rpcTorrentSetLocation(ids, location, move);
    await get().fetchRecentlyActive();
  },

  queueMove: async (ids, direction) => {
    const api = getPlatformApi();
    const methods: Record<string, (ids: number[]) => Promise<unknown>> = {
      top: (ids) => api.rpcQueueMoveTop(ids),
      up: (ids) => api.rpcQueueMoveUp(ids),
      down: (ids) => api.rpcQueueMoveDown(ids),
      bottom: (ids) => api.rpcQueueMoveBottom(ids),
    };
